...
Единый README проекта. Здесь собрана навигация по фронтенду, серверу и модели, а подробные технические заметки по блокам оставлены в отдельных README внутри папок.
...
- [Подробные README блоков](#подробные-readme-блоков)
...
- `client` — интерфейс для ввода текста объявления, просмотра предсказания и работы с сохранёнными сессиями.
- `server` — API и WebSocket-слой для предикта и генерации черновиков.
- `model` — обучение, инференс и экспериментальные скрипты для модели.
...
Если нужен подробный разбор UI/структуры, см. код в `client/src`.
